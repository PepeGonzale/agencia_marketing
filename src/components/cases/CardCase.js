...
export default function CardCase({data, index}) {
    return (
      <Link 
      to="/cases/id"
      onMouseEnter={() => {
                ...
                <div className="flex flex-1 flex-col justify-between bg-white p-6">
                  <div className="flex-1">
                    ...
                    <a href={data.href} className="mt-2 block">
                      <p id={index} className="lg:text-4xl pt-2 pb-6 text-2xl transition duration-400 ease-in-out font-semibold text-gray-900">{data.title}</p>
                      <p className="mt-3 text-xl text-gray-500 leading-9">{data.description}</p>
                    </a>
                  </div>
